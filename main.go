...
func main() {
    ...
    flag.BoolVar(&followRedirects, "f", false, "")

    flag.Parse()

    delay := time.Duration(delayMs * 1000000)
    client := newClient(keepAlives, proxy, followRedirects)
    prefix := outputDir
...
func newClient(keepAlives bool, proxy string, followRedirects bool) *http.Client {
    ...
    if followRedirects {
        re = nil // default behavior is to follow redirects
    } else {
